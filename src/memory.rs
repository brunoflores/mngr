...
/// Returs a mutable reference to the active level 4 table.
///
...
impl BootInfoFrameAllocator {
    ...
    ///
    /// This function is unsafe because the caller must guarantee that the
    /// passed memory map is valid. The main requirement is that all
    /// frames that are maked as `USABLE` in it are really unused.
